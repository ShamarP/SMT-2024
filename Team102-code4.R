...
diagonal_length <- 1.5 * sqrt(2)

right_vertex <- c(second_base_center_x + diagonal_length / 2,second_base_center_y)
bottom_vertex <- c(second_base_center_x,second_base_center_y - diagonal_length / 2)

width <- 1.25

# Calculate the southeast direction vector
hypotenuse_length <- sqrt(2)

right_vertex <- c(right_vertex[1] + width / hypotenuse_length, right_vertex[2]
                  + width / hypotenuse_length )
bottom_vertex <- c(bottom_vertex[1] - width / hypotenuse_length,bottom_vertex[2]
                   - width / hypotenuse_length)


# Calculate the other two points 10 units away
distance <- 10
point3 <- c(right_vertex[1] + distance / hypotenuse_length,right_vertex[2] -
              distance / hypotenuse_length)
point4 <- c(bottom_vertex[1] + distance / hypotenuse_length, bottom_vertex[2] -
              distance / hypotenuse_length)
...
steal_stats <- steals %>%
  select(game_str,play_id,valid_steal,first_baserunner.x,second_baserunner.x,diagonalpitch_time,diagonalexchange_time,diagonalpop_time,valid_steal) %>%
...
tb_right_vertex <- c(third_base_center[1] + diagonal_length / 2,third_base_center[2])
tb_top_vertex <- c(third_base_center[1],third_base_center[2] + diagonal_length / 2)



tb_right_vertex <- c(tb_right_vertex[1] + width / hypotenuse_length,
                     tb_right_vertex[2] - width / hypotenuse_length)
tb_top_vertex <- c(tb_top_vertex[1] - width / hypotenuse_length,
                   tb_top_vertex[2] + width /hypotenuse_length)

pointtbright <- c(tb_right_vertex[1] + distance/hypotenuse_length,
                  tb_right_vertex[2] + distance/hypotenuse_length)
pointtbtop <- c(tb_top_vertex[1] + distance / hypotenuse_length,
                tb_top_vertex[2] + distance / hypotenuse_length)
...
baseball_diamond +
  ...
  ggtitle("Buffer Zones on Baseball Diamond") +
  theme_minimal()
...
write.csv(steal_stats,"Data_csvs/zone_entry.csv")
steal_stats
View(steal_stats)
