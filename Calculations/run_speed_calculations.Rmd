...
speed_calc_first <- function(game_string, play, position) {
  ...
  # level <-substr(game_string, nchar(game_string)-1, nchar(game_string) -1)
...
```{r}
#idea: top 10 fastest baserunners in each minor league division
...
#getting a filtered version of the filtered game info table such that it only includes plays where a steal was attempted
steal_filtered_game_info <- inner_join(filtered_game_info, steal_plays, by=c("game_str" = "game_str",  "play_id" = "play_per_game"))

#merging the filter_steals dataframe with the steal_filtered_game_info
#to include more detailed infomation within the dataframe
...
ggplot(level_four_max_speed_df, aes(x = Max.Speed, y= Player.ID)) + 
  geom_line(color = "blue") +
  geom_point(color = "red" ) +
  labs(title = "Triple A Player Max Speed", x = "X Axis", y = "Y Axis")
```